...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        thisChar = GetComponent<PlayerInfo>();
    ...
    private void Update()
    {
        ...
        if (path != null && currentIndex < path.Count - attackRange)
        {
            ...
            pAnimations.SetMovementAnimation(thisChar.GetMoveSpeed());
    ...
    //If not already going to meeting grid position, set the target gridposition for both Units to their meeting point.
    private void FindTargetMiddle()
    {
        if (targetUnit.CheckIfUnitIsMoving())
        {
            GridPosition playerMeetingPos;
            GridPosition targetMeetingPos;

            (playerMeetingPos, targetMeetingPos) = Pathfinding.Instance
                .FindMeetingPointOfPaths(gridPositions, targetUnit.GetComponent<NPCMovement>().gridPositions);

            if (playerMeetingPos != targetGridPosition)
            {
                targetUnit.GetComponent<NPCMovement>().FindMeetingPointWithTarget(targetGridPosition);

                (path, gridPositions, targetGridPosition) = Pathfinding.Instance.FindGroundTilePath(playerMeetingPos, thisChar);
                Debug.Log("Finding meeting point");
            }
        }
    }
