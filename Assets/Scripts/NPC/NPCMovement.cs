...
public class NPCMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else if (path.Count == currentIndex + attackRange && target != null)
        {
            ...
            {
                readyToAttack = false;
                if ((target as PlayerInfo).GetTargetUnit() == null)
                {
    ...
    public void FindMeetingPointWithTarget(GridPosition gridPosition)
    {
        if (target != null)
        {
            (path, gridPositions, targetGridPosition) = Pathfinding.Instance.FindGroundTilePath(gridPosition, thisNPC);
        }
    }
}