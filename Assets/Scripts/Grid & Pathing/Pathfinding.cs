...
public class Pathfinding : MonoBehaviour
{
    ...
    {
        List<GridPosition> gridPositions = FindPath(thisChar.GetGridPosition(), targetPosition, out _);
    ...
    {
        List<GridPosition> gridPositions = FindPath(thisChar.GetGridPosition(), target.finalGridPosition, out _);
        List<Vector3> pathList = new List<Vector3>();
        GridPosition finalPosition;
        ...
        //This is to ensure the unit stops 1 tile away from the target.
        if (pathList.Count > 1)
        {
    ...
    //This checks if the player and target's paths meet and returns where.
    public (GridPosition playerGridPos, GridPosition targetGridPos) FindMeetingPointOfPaths(List<GridPosition> playerPath, List<GridPosition> targetPath)
    {
        GridPosition playerFinalPosition = playerPath[playerPath.Count - 1];
        GridPosition targetFinalPosition = targetPath[targetPath.Count - 1];

        foreach (GridPosition playerPos in playerPath)
        {
            foreach(GridPosition targetPos in targetPath)
            {
                if (CheckIfNeighbour(playerPos, targetPos))
                {
                    playerFinalPosition = playerPos;
                    targetFinalPosition = targetPos;
                }
            }
        }
        return (playerFinalPosition, targetFinalPosition);
    }
